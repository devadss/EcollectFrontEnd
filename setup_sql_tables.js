...
console.log('🚀 Generating SQL Table DDL Script & Auto-Initializer for Wallet Tables...');
...
// Save in both backend and frontend workspaces
fs.writeFileSync(path.join(basePath, 'CreateWalletTables.sql'), sqlContent, 'utf8');
fs.writeFileSync('CreateWalletTables.sql', sqlContent, 'utf8');
console.log('✅ Created CreateWalletTables.sql in backend and frontend root.');

// 5. Update DbInitializer.cs to execute table creation automatically
const dbInitPath = path.join(basePath, 'Ecollect.Data/DbInitializer.cs');
let dbInit = fs.readFileSync(dbInitPath, 'utf8');

const autoTableCreationSnippet = `
            // ============================================================
            // AUTO-CREATE WALLET TABLES IF NOT PRESENT
            // ============================================================
            try
            {
                await context.Database.ExecuteSqlRawAsync(@"
                    ...
                    BEGIN
                        CREATE TABLE [dbo].[WalletTransactions] (
                            ...
                            [MerchantId] INT NOT NULL,
                            [ReferenceId] NVARCHAR(50) NOT NULL DEFAULT '',
                            [Type] NVARCHAR(20) NOT NULL DEFAULT 'TOPUP',
                            [Amount] DECIMAL(18,2) NOT NULL,
                            [ClosingBalance] DECIMAL(18,2) NOT NULL,
                            [Channel] NVARCHAR(50) NOT NULL DEFAULT 'UPI',
                            [RecipientCount] INT NOT NULL DEFAULT 0,
                            [Notes] NVARCHAR(500) NOT NULL DEFAULT '',
                            [Status] NVARCHAR(20) NOT NULL DEFAULT 'SUCCESS',
                            [CreatedAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE()
            ...
            {
                Console.WriteLine("⚠️ Wallet tables creation note: " + ex.Message);
            }
`;

if (!dbInit.includes('MerchantWallets')) {
    dbInit = dbInit.replace(
        'public static async Task InitializeAsync(ApplicationDbContext context)\r\n        {',
        'public static async Task InitializeAsync(ApplicationDbContext context)\r\n        {' + autoTableCreationSnippet
    );
    if (!dbInit.includes('MerchantWallets')) {
        dbInit = dbInit.replace(
            'public static async Task InitializeAsync(ApplicationDbContext context)\n        {',
    ...
    fs.writeFileSync(dbInitPath, dbInit, 'utf8');
    console.log('✅ Updated DbInitializer.cs with automatic table execution logic.');
} else {
    console.log('ℹ️ DbInitializer.cs already contains auto-create snippet.');
}
